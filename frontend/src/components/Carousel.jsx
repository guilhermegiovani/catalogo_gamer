...
function Carrossel({ items }) {
    ...
    const maxDots = 5
    const half = Math.floor(maxDots / 2)
    const dotSize = 16 + 12 // 16px largura + 12px gap

    // Calcula início da janela dos dots
    let startDot = current - half
    let endDot = current + half + 1

    if (startDot < 0) {
        startDot = 0
        endDot = Math.min(maxDots, items.length)
    }
    if (endDot > items.length) {
        endDot = items.length
        startDot = Math.max(0, endDot - maxDots)
    }

    const visibleDots = items.slice(startDot, endDot)
    const offset = startDot * dotSize
    ...
    return (
        <div className="relative w-full max-w-3xl mx-auto overflow-hidden rounded-lg">
            ...
            <div className="absolute inset-0 pointer-events-none">
                <Button
                    text={<ChevronLeft className="w-4 h-4 landscape:md:h-5 landscape:md:w-5 landscape:xl:h-7 landscape:xl:w-7 portrait:sm:h-6 portrait:sm:w-6" />}
                    className="pointer-events-auto absolute top-1/2 left-2 -translate-y-1/2 bg-black/50 text-white p-1 lg:p-2 rounded-full cursor-pointer z-30 hover:scale-[1.08] aspect-square flex items-center justify-center"
                    handleClick={prevSlide}
                />
                <Button
                    text={<ChevronRight className="w-4 h-4 landscape:md:h-5 landscape:md:w-5 landscape:xl:h-7 landscape:xl:w-7 portrait:sm:h-6 portrait:sm:w-6" />}
                    className="pointer-events-auto absolute top-1/2 right-2 -translate-y-1/2 bg-black/50 text-white p-1 lg:p-2 rounded-full cursor-pointer z-30 hover:scale-[1.08] aspect-square flex items-center justify-center"
                    handleClick={nextSlide}
                />
            </div>

            {/* Pontinhos */}
            <div className="absolute w-full bottom-[-5px] py-4 overflow-hidden">
                <div
                    className="flex gap-3 transition-transform duration-300 ease-in-out justify-center"
                    style={{ transform: `translateX(-${offset}px)` }}
                >
                    {visibleDots.map((item, index) => {
                        ...
                        return (
                            <div
                                key={item.id ?? realIndex}
                                className={clsx(
                                    "rounded-full w-1 h-1 cursor-pointer z-10 transition-transform duration-400",
                                    "landscape:md:w-1.5 landscape:md:h-1.5 landscape:xl:w-2 landscape:xl:h-2",
                                    "portrait:sm:w-2 portrait:sm:h-2",
